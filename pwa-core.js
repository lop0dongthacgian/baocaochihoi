const PWA_CONFIG = {
  APP_NAME:    'BÁO CÁO CÔNG TÁC HỘI VÀ DƯ LUẬN XÃ HỘI',
  APP_DESC:    'Ứng dụng tạo báo cáo chi hội phụ nữ',
...
.pwa-success-overlay{position:fixed;inset:0;z-index:99999;background:rgba(0,0,0,.65);backdrop-filter:blur(16px) saturate(180%);-webkit-backdrop-filter:blur(16px) saturate(180%);display:flex;align-items:center;justify-content:center;padding:20px;animation:pwaSuccessIn .3s ease both;}
@keyframes pwaSuccessIn{from{opacity:0}to{opacity:1}}
.pwa-success-card{background:rgba(255,255,255,.92);backdrop-filter:blur(30px);-webkit-backdrop-filter:blur(30px);border-radius:24px;padding:28px 22px 22px;max-width:340px;width:100%;text-align:center;box-shadow:0 20px 50px rgba(0,0,0,.3),inset 0 1px 0 rgba(255,255,255,.8);border:1px solid rgba(255,255,255,.6);animation:pwaSuccessCard .4s cubic-bezier(.34,1.56,.64,1) both;}
@keyframes pwaSuccessCard{from{opacity:0;transform:scale(.85) translateY(20px);}to{opacity:1;transform:scale(1) translateY(0);}}
.pwa-success-icon{width:70px;height:70px;border-radius:50%;background:linear-gradient(135deg,#4CAF50,#45a049);display:flex;align-items:center;justify-content:center;margin:0 auto 16px;font-size:36px;box-shadow:0 8px 25px rgba(76,175,80,.4);}
.pwa-success-title{font-size:22px;font-weight:800;color:#1e293b;margin-bottom:8px;}
.pwa-success-text{font-size:15px;line-height:1.5;color:rgba(0,0,0,.7);margin-bottom:20px;}
.pwa-success-btn{width:100%;border:none;border-radius:16px;padding:14px;background:linear-gradient(135deg,#4CAF50,#45a049);color:#fff;font-size:17px;font-weight:800;cursor:pointer;box-shadow:0 8px 20px rgba(76,175,80,.35);transition:.18s ease;}
.pwa-success-btn:active{transform:scale(.97);}
.pwa-success-sub{font-size:12px;color:rgba(0,0,0,.5);margin-top:10px;}
`;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  <div id="pwa-card">
    ...
    <div id="pwa-block-android" hidden>
      <div class="pwa-cta-text">Cài app lên <b>Màn hình chính</b><br>để mở nhanh như ứng dụng thật<br><span style="font-size:12px;opacity:.72">Không cần App Store hoặc Google Play</span></div>
      <button id="pwa-btn-install" type="button">📥 Cài đặt ứng dụng</button>
      <div class="pwa-after-note">✅ Sau khi cài xong, mở app từ màn hình chính — không cần vào link này nữa.<br>🗑️ Khi không còn dùng, để gỡ cài đặt: Nhấn giữ icon → <b>Gỡ cài đặt</b></div>
    </div>
  ...
  function showSuccessMessage() {
    ...
    successOverlay.className = 'pwa-success-overlay';
    successOverlay.innerHTML = `
      <div class="pwa-success-card">
        <div class="pwa-success-icon">✅</div>
        <div class="pwa-success-title">${isIOS ? 'Đã thêm vào Màn hình chính!' : 'Cài đặt thành công!'}</div>
        <div class="pwa-success-text">
          ${isIOS 
            ? 'Ứng dụng đã được thêm vào Màn hình chính iPhone của bạn.<br><br>Hãy thoát Safari và mở app từ biểu tượng trên Màn hình chính để sử dụng.' 
            : 'Ứng dụng đã được cài đặt thành công!<br><br>Hãy mở app từ Màn hình chính để sử dụng với trải nghiệm tốt nhất.'}
        </div>
        <button class="pwa-success-btn" id="pwa-success-ok">OK - Thoát trình duyệt</button>
        <div class="pwa-success-sub">(Tab này sẽ tự đóng nếu được hỗ trợ)</div>
      </div>
    `;
    document.body.appendChild(successOverlay);

    document.getElementById('pwa-success-ok').addEventListener('click', () => {
      successOverlay.remove();
      if (isIOS) {
        alert('Vui lòng vuốt lên từ cạnh dưới màn hình để về Màn hình chính, sau đó mở app từ biểu tượng đã thêm.');
      } else {
        try {
          if (/android/i.test(navigator.userAgent)) {
            window.location.href = 'intent://#Intent;scheme=about;package=com.android.chrome;end';
            setTimeout(() => {
              window.close();
              setTimeout(() => {
            ...
            if (!closed || window.closed === false) {
              ...
              msgDiv.style.cssText = 'position:fixed;bottom:20px;left:50%;transform:translateX(-50%);background:#333;color:#fff;padding:12px 24px;border-radius:30px;z-index:999999;font-size:14px;box-shadow:0 4px 20px rgba(0,0,0,.3);';
              msgDiv.textContent = '👆 Cửa sổ đã đóng hoặc nhấn Alt+F4 để thoát';
  ...
  if(!isStandalone){
    ...
    window.addEventListener('beforeinstallprompt',(e)=>{
      ...
      document.getElementById('pwa-btn-install')?.addEventListener('click', async ()=>{
        hideOverlay(); if(!deferredPrompt) return;
        try { deferredPrompt.prompt(); const { outcome } = await deferredPrompt.userChoice; deferredPrompt = null; if (outcome === 'accepted') showSuccessMessage(); } catch(err) { console.log('Install error:', err); }
      },{ once:true });
    });
    window.addEventListener('appinstalled', () => { hideOverlay(); showSuccessMessage(); });
    if(isIOS){
      window.addEventListener('load',()=>{ setTimeout(()=>{ showOverlay('ios'); },PWA_CONFIG.IOS_DELAY); });
      document.addEventListener('visibilitychange', () => { if (document.visibilityState === 'visible' && isStandalone) hideOverlay(); });
    }
  }
});