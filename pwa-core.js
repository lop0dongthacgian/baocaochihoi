...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function setAndroidView(isInstalled){
  ...
  function _checkInstallFallback(){
    var flag = false;
    try { flag = localStorage.getItem('pwa_installed') === '1'; } catch(e){}
    if(flag && window._pwaPromptAvailable){
  ...
  function showOverlay(type){
    if(type === 'ios'){
      ...
      // Kiểm tra 3 trạng thái: chưa cài / đã cài / đã gỡ
      if('getInstalledRelatedApps' in navigator){
        navigator.getInstalledRelatedApps().then(function(apps){
          var real = apps && apps.length > 0;
          if(!real){ try{ localStorage.removeItem('pwa_installed'); }catch(e){} }
          setAndroidView(real);
        }).catch(_checkInstallFallback);
      } else {
        _checkInstallFallback();
      }
      overlayEl.classList.add('pwa-active');
    }
  }
  ...
  window._pwaPromptAvailable = false;
  if(!isStandalone){
