...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // ═══════════════════════════════════════════════════════
  // KIỂM TRA APP ĐÃ ĐƯỢC CÀI CHƯA (REAL-TIME)
  // ═══════════════════════════════════════════════════════
  ...
  // Theo dõi thay đổi display-mode
  const mq = window.matchMedia('(display-mode: standalone)');
  mq.addEventListener('change', (e) => {
    if(e.matches) { try { localStorage.setItem('pwa_installed','1'); } catch(e){} }
    const androidBlock = document.getElementById('pwa-block-android');
    if(androidBlock && !androidBlock.hidden) setAndroidView();
  });
  ...
  window.addEventListener('beforeinstallprompt', (e) => {
    var was = false;
    try { was = localStorage.getItem('pwa_installed') === '1'; } catch(e){}
    if(was){ try { localStorage.removeItem('pwa_installed'); } catch(e){} }
    setAndroidView();
  });
